...
//! ## no_std Port Notes
//!
//! - Replaced `std::collections::HashMap` with `ai_core::HashMap` (hashbrown)
//! - All other logic unchanged (pure data structures)
...
impl MarkovPrefetcher {
    ...
    pub fn record_transition(&mut self, prev_ino: u64, next_ino: u64, next_name: &str) {
        ...
        let transitions = self.transitions.entry(prev_ino).or_insert_with(Vec::new);
        ...
        // New transition
        transitions.push((next_ino, String::from(next_name), 1));
        self.entry_count += 1;
    }
